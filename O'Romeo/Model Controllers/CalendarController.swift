...
class CalendarController {
    ...
    func createCalendarEvent(date: Date, julietName: String, event: String, address: String, personUID: String, description: String, completion: @escaping (Error?) -> Void) {
        ...
        ref = db.collection("calendar").addDocument(data: [
            "date" : date,
            ...
            "description" : description
                ...
                if let error = error {
                    ...
                    PersonController.shared.updatePersonEvents(for: personUID, with: docID, completion: { (error) in
                        if let error = error {
    ...
    func updateEvents(calendar: Calendar, date: Date, julietName: String, event: String, address: String, description: String) {
        db.collection("calendar").document(calendar.calendarUID).updateData([
            "date" : date,
            "julietName" : julietName,
            "event" : event,
            "address" : address,
    ...
    func fetchCalendarEventsFromFirestore(for person: Person, completion: @escaping (Error?) -> Void) {
        db.collection("calendar").whereField("personUID", isEqualTo: person.personUID).getDocuments { (snapshot, error) in
            if let error = error {
            ...
            self.calendarArray = snapshot.documents.compactMap { Calendar(from: $0.data(), uid: $0.documentID) }
            completion(nil)
        }
    }
}
